...
class PageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Datos adicionales para Recibos y Pagos
        $recibosSinPago = [];
        $empleadosList = [];
        $monedas = [];
        $metodos = [];
        ...
        if (!$esEmpleado) {
            ...
            $recibosQuery = DB::table('recibos as r')
                ...
                ->whereNull('p.id')
                ->select('r.id', 'e.nombre', 'e.apellido', 'r.neto', 'pn.codigo as periodo_codigo', 'pn.estado as periodo_estado')
                ->orderByDesc('r.id')
                ->limit(20)
                ->get();
            $recibosSinPago = $recibosQuery;

            // Empleados
            $empleadosList = DB::table('empleados')->select('id','nombre','apellido')->orderBy('nombre')->limit(200)->get();
        ...
        return view('vue.layout', compact(
            'empleados',
            'departamentos',
            'contratos',
            'periodos',
